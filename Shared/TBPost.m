...
- (BOOL)parse:(NSError **)error {	[self loadMarkdownContent]; return ![self parseDateAndSlug:error] ? NO : [self parseTitle], YES; }

- (void)loadMarkdownContent; {

	self.markdownContent =  [NSS stringWithContentsOfURL:self.URL encoding:NSUTF8StringEncoding error:nil];
}
...
- (void)parseTitle {
	...
	NSMS *markdownContent = self.markdownContent.mutableCopy;  static NSRegularExpression *headerRegex = nil;
	headerRegex  = headerRegex ?: [NSRegularExpression regularExpressionWithPattern:@"#[ \\t](.*)[ \\t]#" options:0 error:nil];
	...
	NSS *firstLine = [markdownContent substringWithRange:firstLineRange];
...
- (BOOL)parseDateAndSlug:(NSError **)error {	// Dates and slugs are parsed from a pattern in the post file name.

	static NSRegularExpression *fileNameRegex = nil;
	fileNameRegex   =  fileNameRegex ?: [NSRegularExpression regularExpressionWithPattern:@"^(\\d+-\\d+-\\d+)-(.*)" options:0 error:nil];
	...
	else { if (error) *error = TBError.badPostFileName(self.URL); return NO;	}
	return YES;
}

- (void)parseMarkdownContent {	if (!self.markdownContent || !self.markdownContent.length) return;
