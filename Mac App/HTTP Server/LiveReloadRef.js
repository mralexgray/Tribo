...
// These are like, IN THE INFO.PLIST!
	...
	CustomEvents = {
		bind: function (element, eventName, handler)
		{
			...
			else if (element.attachEvent)
			{
				...
				{
					if (event.propertyName === eventName) return handler();
				});
			}
			else throw new Error("Attempt to attach custom event " + eventName + " to something which isn't a DOMElement");
		},
		fire: function (element, eventName)
		{
			...
			{
				if (element[eventName]) return element[eventName]++;
			}
			else throw new Error("Attempt to fire custom event " + eventName + " on something which isn't a DOMElement");
		}
	};
	...
	__protocol.Parser = Parser = (function ()
	{
		...
		Parser.prototype.process = function (data)
		{
			...
			try
			{
				...
				{
					if (data.match(/^!!ver:([\d.]+)$/)) this.protocol = 6;
