...
@implementation WebViewDelegate  @synthesize sound, dock, growl, notice, path, app, window, requestedWindow;
...
- (void)webView:(WebView*)sender didFinishLoadForFrame:(WebFrame*)frame {
	...
	mainScrollView.hasVerticalScroller				= NO;
...
//	if (self.growl == nil) { self.growl = [Growl new]; }
	...
	if (self.app == nil) self.app = [[App alloc] initWithWebView:webView];
	if (self.window == nil) {
		self.window = [[Window alloc] initWithWebView:webView];
	}

	[windowScriptObject setValue:self forKey:@"AtoZ"];
}
...
- (void)webView:(WebView*)sender frame:(WebFrame*)frame exceededDatabaseQuotaForSecurityOrigin:(id) origin database:(NSString*)databaseIdentifier
{
	...
	SEL setQuota = NSSelectorFromString(@"setQuota:");
	if ([origin respondsToSelector: setQuota]) objc_msgSend(origin, setQuota, @(defaultQuota));
//		[origin performSelector:@selector(setQuota:) withObject:[NSNumber numberWithLongLong: defaultQuota]];
	else NSLog(@"could not increase quota for %llu", defaultQuota);
}
...
- (WebView*)webView:(WebView*)sender createWebViewWithRequest:(NSURLRequest*)request{
	...
	return (requestedWindow = [WindowController.alloc initWithRequest:request]).contentView.webView;
}
- (void)webViewShow:(WebView*)sender{ [requestedWindow showWindow:sender]; }

- (void)webView:(WebView*)webView decidePolicyForNewWindowAction:(NSDictionary*)actionInformation request:(NSURLRequest*)request newFrameName:(NSString*)frameName decisionListener:(id < WebPolicyDecisionListener >)listener
{
	[NSWorkspace.sharedWorkspace openURL:request.URL]; [listener ignore];
}
#pragma mark WebScripting protocol
+ (BOOL) isSelectorExcludedFromWebScript:(SEL)selector	{	return YES; }
